...
export default function MessagePage() {
  ...
  const { data: inboxData } = useQuery({
    queryKey: ['messages', folder, 0, LIMIT],
    queryFn: () => api.messages.list({ folder, limit: LIMIT, offset: 0 }),
  });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
          <div className="flex border-b border-gray-200">
            ...
            >
              Inbox {folder === 'inbox' && total > 0 && `(${total})`}
            </button>
            <button
              type="button"
            ...
            >
              Sent {folder === 'sent' && total > 0 && `(${total})`}
            </button>
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {thread.map((m) => (
          ...
          >
            ...
            {thread.indexOf(m) === 0 && <p className="text-sm font-medium text-gray-700 mb-1">{m.subject}</p>}
